...
function reloadWithMessage(msg, details, hint) {
  ...
  hint = hint || "";
  var newUri = window.location.href.split("#")[0].split("?")[0] + "?bakufu-msg=" + encodeURIComponent(msg) + "&hostname=" + hostname + "&port=" + port + "&hint=" + hint;
...
function createVirtualInstance() {
  var virtualInstance = {
    id: "bakufu-virtual-instance-" + (virtualInstanceCounter++),
    children: [],
    parent: null,
    hasMaster: false,
    inMaintenance: false,
...
$(document).ready(function() {
  ...
  var bakufuMsg = getParameterByName("bakufu-msg")
  if (bakufuMsg) {
    addInfo(bakufuMsg)
    ...
    history.pushState(null, document.title, location.href.split("?bakufu-msg=")[0])
  }
