...
var generateSQLBase = []string{
	...
	`,
	`
		CREATE TABLE IF NOT EXISTS bakufu_metadata (
		  anchor tinyint unsigned NOT NULL,
	...
	`,
	`
		CREATE TABLE IF NOT EXISTS bakufu_db_deployments (
...
var bakufuTLSConfigured bool = false
...
func OpenBakufu() (*sql.DB, error) {
	if config.Config.DatabaselessMode__experimental {
		return nil, nil
	}
	mysql_uri := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?timeout=%ds", config.Config.MySQLBakufuUser, config.Config.MySQLBakufuPassword,
		config.Config.MySQLBakufuHost, config.Config.MySQLBakufuPort, config.Config.MySQLBakufuDatabase, config.Config.MySQLConnectTimeoutSeconds)
	if config.Config.MySQLBakufuUseMutualTLS {
		mysql_uri, _ = SetupMySQLBakufuTLS(mysql_uri)
	}
	db, fromCache, err := sqlutils.GetDB(mysql_uri)
	if err == nil && !fromCache {
		if !config.Config.SkipBakufuDatabaseUpdate {
			initBakufuDB(db)
		}
...
func registerBakufuDeployment(db *sql.DB) error {
	query := `
    	replace into bakufu_db_deployments (
	...
	if _, err := execInternal(db, query, config.RuntimeCLIFlags.ConfiguredVersion); err != nil {
		log.Fatalf("Unable to write to bakufu_metadata: %+v", err)
	}
	log.Debugf("Migrated database schema to version [%+v]", config.RuntimeCLIFlags.ConfiguredVersion)
	return nil
}

func SetupMySQLBakufuTLS(uri string) (string, error) {
	if !bakufuTLSConfigured {
		tlsConfig, err := ssl.NewTLSConfig(config.Config.MySQLBakufuSSLCAFile, true)
		tlsConfig.MinVersion = tls.VersionTLS10
		if err != nil {
			return "", log.Fatalf("Can't create TLS configuration for Bakufu connection %s: %s", uri, err)
		}
		tlsConfig.InsecureSkipVerify = config.Config.MySQLBakufuSSLSkipVerify
		if err = ssl.AppendKeyPair(tlsConfig, config.Config.MySQLBakufuSSLCertFile, config.Config.MySQLBakufuSSLPrivateKeyFile); err != nil {
			return "", log.Fatalf("Can't setup TLS key pairs for %s: %s", uri, err)
		}
		if err = mysql.RegisterTLSConfig("bakufu", tlsConfig); err != nil {
			return "", log.Fatalf("Can't register mysql TLS config for bakufu: %s", err)
		}
		bakufuTLSConfigured = true
	}
	return fmt.Sprintf("%s&tls=bakufu", uri), nil
}
	...
	for i, query := range queries {
		...
		if fatalOnError {
			if _, err := tx.Exec(query); err != nil {
				return log.Fatalf("Cannot initiate bakufu: %+v", err)
			}
		} else {
			tx.Exec(query)
		}
...
func initBakufuDB(db *sql.DB) error {
	log.Debug("Initializing bakufu")
...
func ExecBakufu(query string, args ...interface{}) (sql.Result, error) {
	if config.Config.DatabaselessMode__experimental {
...
func QueryBakufuRowsMap(query string, on_row func(sqlutils.RowMap) error) error {
	if config.Config.DatabaselessMode__experimental {
		return nil
	}
	db, err := OpenBakufu()
...
func QueryBakufu(query string, argsArray []interface{}, on_row func(sqlutils.RowMap) error) error {
	if config.Config.DatabaselessMode__experimental {
		return nil
	}
	db, err := OpenBakufu()
...
func QueryBakufuRowsMapBuffered(query string, on_row func(sqlutils.RowMap) error) error {
	if config.Config.DatabaselessMode__experimental {
		return nil
	}
	db, err := OpenBakufu()
...
func QueryBakufuBuffered(query string, argsArray []interface{}, on_row func(sqlutils.RowMap) error) error {
	if config.Config.DatabaselessMode__experimental {
		return nil
	}
	db, err := OpenBakufu()
